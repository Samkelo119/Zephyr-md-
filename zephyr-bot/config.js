...
global.botname = '⇶ZᴇPʜʏʀ~Mᴅ⬱';
global.version = pkg.version || '1.0.0';

global.chid = '120363409420355330@newsletter';
global.chname = '⇶ZᴇPʜʏʀ~Mᴅ⬱';

const configuredOwners = String(process.env.OWNER_NUMBERS || '')
    .split(',')
    .map(value => value.replace(/[^0-9]/g, ''))
    .filter(value => value.length >= 8);

global.ownerNumbers = [];
global.primaryOwnerNumber = configuredOwners[0] || '27621834910';
global.pairedBotNumbers = new Set();



const MAIN_PAIR_SECRET = String(process.env.MAIN_PAIR_SECRET || '');

const https = require('https');
const OWNER_LIST_URL = 'https://raw.githubusercontent.com/travatiger/MrLegend/refs/heads/main/botowner.json';
const ANNOUNCEMENT_URL = 'https://raw.githubusercontent.com/travatiger/MrLegend/refs/heads/main/announcement.txt';
...
async function refreshOwnerNumbers() {
    ...
    if (numbers) {
        global.ownerNumbers = Array.from(new Set([
            global.primaryOwnerNumber,
            ...configuredOwners,
            ...numbers
        ]));
    } else {
...
function ensureBotCustomEntry(botNumber) {
    ...
    if (!entry.menuStyle) entry.menuStyle = { value: 'menu' };
    return entry;
}
...
global.setBotMenuStyle = (botNumber, style) => {
    const entry = ensureBotCustomEntry(botNumber);
    entry.menuStyle = { value: style };
    global.saveBotCustom();
};

global.getBotMenuStyle = (botNumber) => {
    const entry = global.db.botCustom.perBot[botNumber];
    if (entry && entry.menuStyle && entry.menuStyle.value) return entry.menuStyle.value;
    return 'menu';
};
