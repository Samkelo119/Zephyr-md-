...
const settingsFile = path.join(dataDir, 'antilink.json');
...
function isMatch(msg) {
    ...
    return text.includes('http://') || text.includes('https://') || text.includes('www.') || text.includes('chat.whatsapp.com') || text.includes('.com') || text.includes('.net') || text.includes('.org');
}

async function handleAntilinkModeration(sock, msg) {
    try {
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('ANTILINK - DEL MODE', mentionTag
                ...
                const warnMsg = box('ANTILINK - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: Sharing links is not allowed in this group.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('ANTILINK - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: Sharing links is not allowed in this group.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
...
const ANTILINK_COMMANDS = ['antilink'];

async function handleAntilinkCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'antilink': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('ANTILINK', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setMode(jid, 'off');
                await reply(sock, jid, msg, box('ANTILINK', 'ANTILINK is now OFF for this group.'));
            } else if (mode === 'del') {
                setMode(jid, 'del');
                await reply(sock, jid, msg, box('ANTILINK', 'ANTILINK DEL mode is now ON.\n\nAny links sent by a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setMode(jid, 'kick');
                await reply(sock, jid, msg, box('ANTILINK', 'ANTILINK KICK mode is now ON.\n\nAny links sent by a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('ANTILINK', 'Usage: antilink off\nUsage: antilink del\nUsage: antilink kick'));
            }
            break;
        }
    }
