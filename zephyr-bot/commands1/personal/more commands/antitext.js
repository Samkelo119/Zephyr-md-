...
const settingsFile = path.join(dataDir, 'antitext.json');
...
function isMatch(msg) {
    ...
    return !!(msg.message.conversation || msg.message.extendedTextMessage);
}

async function handleAntitextModeration(sock, msg) {
    try {
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('ANTITEXT - DEL MODE', mentionTag
                ...
                const warnMsg = box('ANTITEXT - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: Sending plain text messages is not allowed in this group.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('ANTITEXT - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: Sending plain text messages is not allowed in this group.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
        ...
        console.log('ANTITEXT error:', e.message);
    }
}

const ANTITEXT_COMMANDS = ['antitext'];

async function handleAntitextCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'antitext': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('ANTITEXT', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setMode(jid, 'off');
                await reply(sock, jid, msg, box('ANTITEXT', 'ANTITEXT is now OFF for this group.'));
            } else if (mode === 'del') {
                setMode(jid, 'del');
                await reply(sock, jid, msg, box('ANTITEXT', 'ANTITEXT DEL mode is now ON.\n\nAny plain text messages sent by a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setMode(jid, 'kick');
                await reply(sock, jid, msg, box('ANTITEXT', 'ANTITEXT KICK mode is now ON.\n\nAny plain text messages sent by a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('ANTITEXT', 'Usage: antitext off\nUsage: antitext del\nUsage: antitext kick'));
            }
            break;
        }
    }
...
module.exports = {
    handleAntitextCommand,
    handleAntitextModeration,
    ANTITEXT_COMMANDS
};
