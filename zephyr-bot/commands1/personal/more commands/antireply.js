...
const settingsFile = path.join(dataDir, 'antireply.json');
...
async function handleAntireplyModeration(sock, msg) {
    try {
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('ANTIREPLY - DEL MODE', mentionTag
                ...
                const warnMsg = box('ANTIREPLY - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: Sharing replies is not allowed in this group.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('ANTIREPLY - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: Sharing replies is not allowed in this group.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
        ...
        console.log('ANTIREPLY error:', e.message);
    }
}

const ANTIREPLY_COMMANDS = ['antireply'];

async function handleAntireplyCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'antireply': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('ANTIREPLY', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setMode(jid, 'off');
                await reply(sock, jid, msg, box('ANTIREPLY', 'ANTIREPLY is now OFF for this group.'));
            } else if (mode === 'del') {
                setMode(jid, 'del');
                await reply(sock, jid, msg, box('ANTIREPLY', 'ANTIREPLY DEL mode is now ON.\n\nAny replies sent by a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setMode(jid, 'kick');
                await reply(sock, jid, msg, box('ANTIREPLY', 'ANTIREPLY KICK mode is now ON.\n\nAny replies sent by a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('ANTIREPLY', 'Usage: antireply off\nUsage: antireply del\nUsage: antireply kick'));
            }
            break;
        }
    }
...
module.exports = {
    handleAntireplyCommand,
    handleAntireplyModeration,
    ANTIREPLY_COMMANDS
};
