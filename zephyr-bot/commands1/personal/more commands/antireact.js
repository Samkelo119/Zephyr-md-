...
const settingsFile = path.join(dataDir, 'antireact.json');
...
function isMatch(msg) {
    return !!(msg.message && msg.message.reactionMessage);
}

async function handleAntireactModeration(sock, msg) {
    try {
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('ANTIREACT - DEL MODE', mentionTag
                ...
                const warnMsg = box('ANTIREACT - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: Sharing reactions is not allowed in this group.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('ANTIREACT - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: Sharing reactions is not allowed in this group.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
        ...
        console.log('ANTIREACT error:', e.message);
    }
}

const ANTIREACT_COMMANDS = ['antireact'];

async function handleAntireactCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'antireact': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('ANTIREACT', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setMode(jid, 'off');
                await reply(sock, jid, msg, box('ANTIREACT', 'ANTIREACT is now OFF for this group.'));
            } else if (mode === 'del') {
                setMode(jid, 'del');
                await reply(sock, jid, msg, box('ANTIREACT', 'ANTIREACT DEL mode is now ON.\n\nAny reactions sent by a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setMode(jid, 'kick');
                await reply(sock, jid, msg, box('ANTIREACT', 'ANTIREACT KICK mode is now ON.\n\nAny reactions sent by a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('ANTIREACT', 'Usage: antireact off\nUsage: antireact del\nUsage: antireact kick'));
            }
            break;
        }
    }
...
module.exports = {
    handleAntireactCommand,
    handleAntireactModeration,
    ANTIREACT_COMMANDS
};
