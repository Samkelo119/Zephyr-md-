...
const settingsFile = path.join(dataDir, 'antiaudio.json');
...
function isMatch(msg) {
    return !!(msg.message && msg.message.audioMessage);
}

async function handleAntiaudioModeration(sock, msg) {
    try {
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('ANTIAUDIO - DEL MODE', mentionTag
                ...
                const warnMsg = box('ANTIAUDIO - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: Sharing audio messages is not allowed in this group.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('ANTIAUDIO - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: Sharing audio messages is not allowed in this group.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
        ...
        console.log('ANTIAUDIO error:', e.message);
    }
}

const ANTIAUDIO_COMMANDS = ['antiaudio'];

async function handleAntiaudioCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'antiaudio': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('ANTIAUDIO', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setMode(jid, 'off');
                await reply(sock, jid, msg, box('ANTIAUDIO', 'ANTIAUDIO is now OFF for this group.'));
            } else if (mode === 'del') {
                setMode(jid, 'del');
                await reply(sock, jid, msg, box('ANTIAUDIO', 'ANTIAUDIO DEL mode is now ON.\n\nAny audio messages sent by a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setMode(jid, 'kick');
                await reply(sock, jid, msg, box('ANTIAUDIO', 'ANTIAUDIO KICK mode is now ON.\n\nAny audio messages sent by a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('ANTIAUDIO', 'Usage: antiaudio off\nUsage: antiaudio del\nUsage: antiaudio kick'));
            }
            break;
        }
    }
...
module.exports = {
    handleAntiaudioCommand,
    handleAntiaudioModeration,
    ANTIAUDIO_COMMANDS
};
