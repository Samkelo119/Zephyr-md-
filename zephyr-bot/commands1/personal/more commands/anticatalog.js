...
const settingsFile = path.join(dataDir, 'anticatalog.json');
...
function isMatch(msg) {
    return !!(msg.message && (msg.message.productMessage || msg.message.catalogMessage));
}

async function handleAnticatalogModeration(sock, msg) {
    try {
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('ANTICATALOG - DEL MODE', mentionTag
                ...
                const warnMsg = box('ANTICATALOG - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: Sharing catalog messages is not allowed in this group.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('ANTICATALOG - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: Sharing catalog messages is not allowed in this group.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
        ...
        console.log('ANTICATALOG error:', e.message);
    }
}

const ANTICATALOG_COMMANDS = ['anticatalog'];

async function handleAnticatalogCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'anticatalog': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('ANTICATALOG', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setMode(jid, 'off');
                await reply(sock, jid, msg, box('ANTICATALOG', 'ANTICATALOG is now OFF for this group.'));
            } else if (mode === 'del') {
                setMode(jid, 'del');
                await reply(sock, jid, msg, box('ANTICATALOG', 'ANTICATALOG DEL mode is now ON.\n\nAny catalog messages sent by a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setMode(jid, 'kick');
                await reply(sock, jid, msg, box('ANTICATALOG', 'ANTICATALOG KICK mode is now ON.\n\nAny catalog messages sent by a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('ANTICATALOG', 'Usage: anticatalog off\nUsage: anticatalog del\nUsage: anticatalog kick'));
            }
            break;
        }
    }
...
module.exports = {
    handleAnticatalogCommand,
    handleAnticatalogModeration,
    ANTICATALOG_COMMANDS
};
