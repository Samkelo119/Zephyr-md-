...
const settingsFile = path.join(dataDir, 'anticontact.json');
...
function isMatch(msg) {
    return !!(msg.message && (msg.message.contactMessage || msg.message.contactsArrayMessage));
}

async function handleAnticontactModeration(sock, msg) {
    try {
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('ANTICONTACT - DEL MODE', mentionTag
                ...
                const warnMsg = box('ANTICONTACT - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: Sharing contact cards is not allowed in this group.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('ANTICONTACT - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: Sharing contact cards is not allowed in this group.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
        ...
        console.log('ANTICONTACT error:', e.message);
    }
}

const ANTICONTACT_COMMANDS = ['anticontact'];

async function handleAnticontactCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'anticontact': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('ANTICONTACT', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setMode(jid, 'off');
                await reply(sock, jid, msg, box('ANTICONTACT', 'ANTICONTACT is now OFF for this group.'));
            } else if (mode === 'del') {
                setMode(jid, 'del');
                await reply(sock, jid, msg, box('ANTICONTACT', 'ANTICONTACT DEL mode is now ON.\n\nAny contact cards sent by a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setMode(jid, 'kick');
                await reply(sock, jid, msg, box('ANTICONTACT', 'ANTICONTACT KICK mode is now ON.\n\nAny contact cards sent by a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('ANTICONTACT', 'Usage: anticontact off\nUsage: anticontact del\nUsage: anticontact kick'));
            }
            break;
        }
    }
...
module.exports = {
    ...
    handleAnticontactModeration,
    ANTICONTACT_COMMANDS
};
