...
const settingsFile = path.join(dataDir, 'antidocument.json');
...
function isMatch(msg) {
    return !!(msg.message && (msg.message.documentMessage || msg.message.documentWithCaptionMessage));
}

async function handleAntidocumentModeration(sock, msg) {
    try {
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('ANTIDOCUMENT - DEL MODE', mentionTag
                ...
                const warnMsg = box('ANTIDOCUMENT - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: Sharing documents is not allowed in this group.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('ANTIDOCUMENT - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: Sharing documents is not allowed in this group.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
        ...
        console.log('ANTIDOCUMENT error:', e.message);
    }
}

const ANTIDOCUMENT_COMMANDS = ['antidocument'];

async function handleAntidocumentCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'antidocument': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('ANTIDOCUMENT', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setMode(jid, 'off');
                await reply(sock, jid, msg, box('ANTIDOCUMENT', 'ANTIDOCUMENT is now OFF for this group.'));
            } else if (mode === 'del') {
                setMode(jid, 'del');
                await reply(sock, jid, msg, box('ANTIDOCUMENT', 'ANTIDOCUMENT DEL mode is now ON.\n\nAny documents sent by a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setMode(jid, 'kick');
                await reply(sock, jid, msg, box('ANTIDOCUMENT', 'ANTIDOCUMENT KICK mode is now ON.\n\nAny documents sent by a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('ANTIDOCUMENT', 'Usage: antidocument off\nUsage: antidocument del\nUsage: antidocument kick'));
            }
            break;
        }
    }
...
module.exports = {
    handleAntidocumentCommand,
    handleAntidocumentModeration,
    ANTIDOCUMENT_COMMANDS
};
