...
const settingsFile = path.join(dataDir, 'gplock.json');
...
function getGplockMode(jid) {
    return settings.modes[jid] || 'off';
}

function setGplockMode(jid, mode) {
...
async function handleGplockModeration(sock, msg) {
    try {
        const jid = msg.key.remoteJid;
        if (!jid || !jid.endsWith('@g.us')) return;
        if (msg.key.fromMe) return;

        const mode = getGplockMode(jid);
        if (mode === 'off') return;
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('GPLOCK - DEL MODE', mentionTag
                ...
                const warnMsg = box('GPLOCK - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: All posts are being filtered.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('GPLOCK - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: All posts are being filtered.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
        ...
        console.log('GPLOCK error:', e.message);
    }
}

const GPLOCK_COMMANDS = ['gplock'];

async function handleGplockCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'gplock': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('GPLOCK', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setGplockMode(jid, 'off');
                await reply(sock, jid, msg, box('GPLOCK', 'Gplock is now OFF for this group.'));
            } else if (mode === 'del') {
                setGplockMode(jid, 'del');
                await reply(sock, jid, msg, box('GPLOCK', 'Gplock DEL mode is now ON.\n\nAny post from a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setGplockMode(jid, 'kick');
                await reply(sock, jid, msg, box('GPLOCK', 'Gplock KICK mode is now ON.\n\nAny post from a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('GPLOCK', 'Usage: gplock off\nUsage: gplock del\nUsage: gplock kick'));
            }
            break;
        }
    }
