...
const settingsFile = path.join(dataDir, 'antipoll.json');
...
function isMatch(msg) {
    return !!(msg.message && (msg.message.pollCreationMessage || msg.message.pollCreationMessageV2 || msg.message.pollCreationMessageV3));
}

async function handleAntipollModeration(sock, msg) {
    try {
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('ANTIPOLL - DEL MODE', mentionTag
                ...
                const warnMsg = box('ANTIPOLL - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: Sharing polls is not allowed in this group.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('ANTIPOLL - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: Sharing polls is not allowed in this group.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
        ...
        console.log('ANTIPOLL error:', e.message);
    }
}

const ANTIPOLL_COMMANDS = ['antipoll'];

async function handleAntipollCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'antipoll': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('ANTIPOLL', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setMode(jid, 'off');
                await reply(sock, jid, msg, box('ANTIPOLL', 'ANTIPOLL is now OFF for this group.'));
            } else if (mode === 'del') {
                setMode(jid, 'del');
                await reply(sock, jid, msg, box('ANTIPOLL', 'ANTIPOLL DEL mode is now ON.\n\nAny polls sent by a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setMode(jid, 'kick');
                await reply(sock, jid, msg, box('ANTIPOLL', 'ANTIPOLL KICK mode is now ON.\n\nAny polls sent by a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('ANTIPOLL', 'Usage: antipoll off\nUsage: antipoll del\nUsage: antipoll kick'));
            }
            break;
        }
    }
...
module.exports = {
    handleAntipollCommand,
    handleAntipollModeration,
    ANTIPOLL_COMMANDS
};
