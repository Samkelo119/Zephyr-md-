...
const settingsFile = path.join(dataDir, 'antisticker.json');
...
function isMatch(msg) {
    return !!(msg.message && msg.message.stickerMessage);
}

async function handleAntistickerModeration(sock, msg) {
    try {
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('ANTISTICKER - DEL MODE', mentionTag
                ...
                const warnMsg = box('ANTISTICKER - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: Sharing stickers is not allowed in this group.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('ANTISTICKER - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: Sharing stickers is not allowed in this group.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
        ...
        console.log('ANTISTICKER error:', e.message);
    }
}

const ANTISTICKER_COMMANDS = ['antisticker'];

async function handleAntistickerCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'antisticker': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('ANTISTICKER', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setMode(jid, 'off');
                await reply(sock, jid, msg, box('ANTISTICKER', 'ANTISTICKER is now OFF for this group.'));
            } else if (mode === 'del') {
                setMode(jid, 'del');
                await reply(sock, jid, msg, box('ANTISTICKER', 'ANTISTICKER DEL mode is now ON.\n\nAny stickers sent by a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setMode(jid, 'kick');
                await reply(sock, jid, msg, box('ANTISTICKER', 'ANTISTICKER KICK mode is now ON.\n\nAny stickers sent by a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('ANTISTICKER', 'Usage: antisticker off\nUsage: antisticker del\nUsage: antisticker kick'));
            }
            break;
        }
    }
...
module.exports = {
    ...
    handleAntistickerModeration,
    ANTISTICKER_COMMANDS
};
