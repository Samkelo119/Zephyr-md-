...
const settingsFile = path.join(dataDir, 'antievent.json');
...
function isMatch(msg) {
    return !!(msg.message && msg.message.eventMessage);
}

async function handleAntieventModeration(sock, msg) {
    try {
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('ANTIEVENT - DEL MODE', mentionTag
                ...
                const warnMsg = box('ANTIEVENT - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: Sharing events is not allowed in this group.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('ANTIEVENT - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: Sharing events is not allowed in this group.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
        ...
        console.log('ANTIEVENT error:', e.message);
    }
}

const ANTIEVENT_COMMANDS = ['antievent'];

async function handleAntieventCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'antievent': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('ANTIEVENT', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setMode(jid, 'off');
                await reply(sock, jid, msg, box('ANTIEVENT', 'ANTIEVENT is now OFF for this group.'));
            } else if (mode === 'del') {
                setMode(jid, 'del');
                await reply(sock, jid, msg, box('ANTIEVENT', 'ANTIEVENT DEL mode is now ON.\n\nAny events sent by a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setMode(jid, 'kick');
                await reply(sock, jid, msg, box('ANTIEVENT', 'ANTIEVENT KICK mode is now ON.\n\nAny events sent by a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('ANTIEVENT', 'Usage: antievent off\nUsage: antievent del\nUsage: antievent kick'));
            }
            break;
        }
    }
...
module.exports = {
    handleAntieventCommand,
    handleAntieventModeration,
    ANTIEVENT_COMMANDS
};
