...
const settingsFile = path.join(dataDir, 'antiemoji.json');
...
async function handleAntiemojiModeration(sock, msg) {
    try {
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('ANTIEMOJI - DEL MODE', mentionTag
                ...
                const warnMsg = box('ANTIEMOJI - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: Sharing emojis is not allowed in this group.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('ANTIEMOJI - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: Sharing emojis is not allowed in this group.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
        ...
        console.log('ANTIEMOJI error:', e.message);
    }
}

const ANTIEMOJI_COMMANDS = ['antiemoji'];

async function handleAntiemojiCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'antiemoji': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('ANTIEMOJI', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setMode(jid, 'off');
                await reply(sock, jid, msg, box('ANTIEMOJI', 'ANTIEMOJI is now OFF for this group.'));
            } else if (mode === 'del') {
                setMode(jid, 'del');
                await reply(sock, jid, msg, box('ANTIEMOJI', 'ANTIEMOJI DEL mode is now ON.\n\nAny emojis sent by a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setMode(jid, 'kick');
                await reply(sock, jid, msg, box('ANTIEMOJI', 'ANTIEMOJI KICK mode is now ON.\n\nAny emojis sent by a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('ANTIEMOJI', 'Usage: antiemoji off\nUsage: antiemoji del\nUsage: antiemoji kick'));
            }
            break;
        }
    }
...
module.exports = {
    handleAntiemojiCommand,
    handleAntiemojiModeration,
    ANTIEMOJI_COMMANDS
};
