...
const settingsFile = path.join(dataDir, 'antilocation.json');
...
function isMatch(msg) {
    return !!(msg.message && (msg.message.locationMessage || msg.message.liveLocationMessage));
}

async function handleAntilocationModeration(sock, msg) {
    try {
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('ANTILOCATION - DEL MODE', mentionTag
                ...
                const warnMsg = box('ANTILOCATION - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: Sharing locations is not allowed in this group.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('ANTILOCATION - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: Sharing locations is not allowed in this group.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
        ...
        console.log('ANTILOCATION error:', e.message);
    }
}

const ANTILOCATION_COMMANDS = ['antilocation'];

async function handleAntilocationCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'antilocation': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('ANTILOCATION', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setMode(jid, 'off');
                await reply(sock, jid, msg, box('ANTILOCATION', 'ANTILOCATION is now OFF for this group.'));
            } else if (mode === 'del') {
                setMode(jid, 'del');
                await reply(sock, jid, msg, box('ANTILOCATION', 'ANTILOCATION DEL mode is now ON.\n\nAny locations sent by a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setMode(jid, 'kick');
                await reply(sock, jid, msg, box('ANTILOCATION', 'ANTILOCATION KICK mode is now ON.\n\nAny locations sent by a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('ANTILOCATION', 'Usage: antilocation off\nUsage: antilocation del\nUsage: antilocation kick'));
            }
            break;
        }
    }
...
module.exports = {
    handleAntilocationCommand,
    handleAntilocationModeration,
    ANTILOCATION_COMMANDS
};
