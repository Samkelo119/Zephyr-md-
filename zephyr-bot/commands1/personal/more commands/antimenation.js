...
const settingsFile = path.join(dataDir, 'antimenation.json');
...
function isMatch(msg) {
    ...
    return !!(ctx && ctx.mentionedJid && ctx.mentionedJid.length > 0);
}

async function handleAntimenationModeration(sock, msg) {
    try {
        ...
        if (mode === 'del') {
            ...
            if (count >= MAX_WARNINGS) {
                const kickMsg = box('ANTIMENTION - DEL MODE', mentionTag
                ...
                const warnMsg = box('ANTIMENTION - DEL MODE', mentionTag
                    + '\n\n🚫 Message deleted\nReason: Sharing mentions is not allowed in this group.'
                    + '\n⚠️ Warning ' + count + '/' + MAX_WARNINGS);
                await reply(sock, jid, msg, warnMsg, { mentions: [realJid] });
            }
        } else if (mode === 'kick') {
            const kickMsg = box('ANTIMENTION - KICK MODE', mentionTag
                + '\n\n🚫 Message deleted\nReason: Sharing mentions is not allowed in this group.'
                + '\n👋 You are being removed from the group.');
            await reply(sock, jid, msg, kickMsg, { mentions: [realJid] });
        ...
        console.log('ANTIMENTION error:', e.message);
    }
}

const ANTIMENATION_COMMANDS = ['antimenation'];

async function handleAntimenationCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    switch (command) {
        case 'antimenation': {
            if (!jid.endsWith('@g.us')) {
                await reply(sock, jid, msg, box('ANTIMENTION', 'This command only works inside groups.'));
                break;
            }
            ...
            if (mode === 'off') {
                setMode(jid, 'off');
                await reply(sock, jid, msg, box('ANTIMENTION', 'ANTIMENTION is now OFF for this group.'));
            } else if (mode === 'del') {
                setMode(jid, 'del');
                await reply(sock, jid, msg, box('ANTIMENTION', 'ANTIMENTION DEL mode is now ON.\n\nAny mentions sent by a non-admin member will be deleted and warned.\nAfter ' + MAX_WARNINGS + ' warnings, the member will be removed.'));
            } else if (mode === 'kick') {
                setMode(jid, 'kick');
                await reply(sock, jid, msg, box('ANTIMENTION', 'ANTIMENTION KICK mode is now ON.\n\nAny mentions sent by a non-admin member will be deleted and the member will be removed immediately.'));
            } else {
                await reply(sock, jid, msg, box('ANTIMENTION', 'Usage: antimenation off\nUsage: antimenation del\nUsage: antimenation kick'));
            }
            break;
        }
    }
...
module.exports = {
    handleAntimenationCommand,
    handleAntimenationModeration,
    ANTIMENATION_COMMANDS
};
