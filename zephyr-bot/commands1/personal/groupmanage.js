...
module.exports = {
    name: 'groupmanage',
    aliases: ['setgdesc', 'setgname', 'setgphoto', 'gmanage'],
    description: 'Manage group settings (name, description, photo)',
    execute: async (sock, remoteJid, message, args) => {
        const senderId = message.key.participant || message.key.remoteJid;
        const sub = (args[0] || '').toLowerCase();
        const text = args.slice(1).join(' ');
        if (sub === 'setgdesc' || sub === 'desc') {
            return setGroupDescription(sock, remoteJid, senderId, text, message);
        } else if (sub === 'setgname' || sub === 'name') {
            return setGroupName(sock, remoteJid, senderId, text, message);
        } else if (sub === 'setgphoto' || sub === 'photo') {
            return setGroupPhoto(sock, remoteJid, senderId, message);
        } else {
            return sock.sendMessage(remoteJid, {
                text: '*GROUP MANAGE*\n\n*.groupmanage desc <text>* - Set group description\n*.groupmanage name <text>* - Set group name\n*.groupmanage photo* - Set group photo (reply to image)'
            }, { quoted: message });
        }
    },
    setGroupDescription,
