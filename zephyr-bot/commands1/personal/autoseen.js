...
const settingsFile = path.join(dataDir, 'autoseen.json');
...
async function handleAutoseenWatch(sock, msg) {
    try {
        ...
        console.log('AUTOSEEN error:', e.message);
    }
}

async function handleAutoseenCommand(sock, jid, msg, command, params, sender, senderIsOwner) {
    if (command === 'autoseen' || command === 'autoread') {
            if (!senderIsOwner) {
            ...
            if (mode === 'on') {
                ...
                await reply(sock, jid, msg, box('AUTOSEEN', 'AUTOSEEN is now ON.\n\nThe bot will automatically read every incoming message.'));
            } else if (mode === 'off') {
                setEnabled(botNumber, false);
                await reply(sock, jid, msg, box('AUTOSEEN', 'AUTOSEEN is now OFF.\n\nThe bot will stop automatically reading incoming messages.'));
            } else {
                const state = isEnabled(botNumber) ? 'ON' : 'OFF';
                await reply(sock, jid, msg, box('AUTOSEEN', 'Current status: ' + state + '\n\nUsage: autoseen on\nUsage: autoseen off'));
            }
            return true;
    }
    return false;
