...
async function handleChatbotCommand(sock, chatId, message, args, context) {
    const match = args[0];
    if (!match) {
    ...
    if (isOwner) {
        if (match === "on") {
            ...
            if (data.chatbot[chatId]) {
                return sock.sendMessage(chatId, { 
                    text: "*_Chatbot is already enabled for this group_*",
                    quoted: message
                });
            }
            ...
            return sock.sendMessage(chatId, { 
                text: "*_Chatbot has been enabled for this group_*",
                quoted: message
            });
        }
        ...
        if (match === "off") {
            ...
            if (!data.chatbot[chatId]) {
                return sock.sendMessage(chatId, { 
                    text: "*_Chatbot is already disabled for this group_*",
                    quoted: message
                });
            }
            ...
            return sock.sendMessage(chatId, { 
                text: "*_Chatbot has been disabled for this group_*",
                quoted: message
            });
        }
    }
    ...
    await showTyping(sock, chatId);
    return sock.sendMessage(chatId, {
        text: "*_Invalid command. Use .chatbot to see usage_*",
        quoted: message
    });
}
...
module.exports = {
    name: "chatbot",
    aliases: ["cb", "ai"],
    description: "Enables or disables the chatbot in a group.",
    execute: async (sock, remoteJid, message, args, context) => {
        return handleChatbotCommand(sock, remoteJid, message, args, context);
    },
};
