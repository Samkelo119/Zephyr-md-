...
module.exports = {
  ...
  adminOnly: false,
  groupOnly: false,
  description: 'Apply the squirrel effect to a voice note. Reply to an audio message with .squirrel',
  async run({ sock, msg, jid }) {
    ...
    if (!target.message?.audioMessage) {
      await sock.sendMessage(jid, { text: 'Reply to a voice note or audio file with .squirrel' }, { quoted: msg });
      return;
    }

    try {
      const buffer = await downloadMediaMessage(target, 'buffer', {});
      const output = await applyAudioFilter(buffer, FILTERS.squirrel);
