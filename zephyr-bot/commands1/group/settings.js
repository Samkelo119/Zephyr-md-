...
async function settingsCommand(sock, chatId, message, context) {
    try {
        const senderId = message.key.participant || message.key.remoteJid;
        const isOwner = await isOwnerOrSudo(senderId, sock, chatId);
        
        if (!message.key.fromMe && !isOwner) {
            await context.sendResponse(sock, chatId, 'Only bot owner can use this command!', message);
            return;
        }
        ...
        const mode = global.botMode || { isPrivate: false };
        ...
        lines.push('');
        lines.push(`• Mode: ${mode.isPrivate ? 'Private' : 'Public'}`);
        ...
        await context.sendResponse(sock, chatId, lines.join('\n'), message);
    } catch (error) {
        console.error('Error in settings command:', error);
        await context.sendResponse(sock, chatId, 'Failed to read settings.', message);
    }
}
...
module.exports = {
    ...
    execute: async (sock, remoteJid, message, args, context) => {
        return settingsCommand(sock, remoteJid, message, context);
    }
};
