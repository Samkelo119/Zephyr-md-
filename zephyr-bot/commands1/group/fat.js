...
module.exports = {
  ...
  adminOnly: false,
  groupOnly: false,
  description: 'Apply the fat effect to a voice note. Reply to an audio message with .fat',
  async run({ sock, msg, jid }) {
    ...
    if (!target.message?.audioMessage) {
      await sock.sendMessage(jid, { text: 'Reply to a voice note or audio file with .fat' }, { quoted: msg });
      return;
    }

    try {
      const buffer = await downloadMediaMessage(target, 'buffer', {});
      const output = await applyAudioFilter(buffer, FILTERS.fat);
