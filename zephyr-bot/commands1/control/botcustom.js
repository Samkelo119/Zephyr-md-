...
function notSuperOwnerMsg() {
    return 'Only the bot\'s own number or the official GitHub owner list can use this command.';
}
...
async function handleSetBotMediaCommand(sock, jid, msg, type, botNumber, senderIsSuperOwner) {
    const meta = MEDIA_TYPES[type];
    if (!senderIsSuperOwner) {
        await reply(sock, jid, msg, box(meta.label, notSuperOwnerMsg()));
        return;
    }
...
async function handleRestartMediaCommand(sock, jid, msg, type, botNumber, senderIsSuperOwner) {
    const meta = MEDIA_TYPES[type];
    if (!senderIsSuperOwner) {
        await reply(sock, jid, msg, box(meta.label, notSuperOwnerMsg()));
        return;
...
async function handleToggleMediaCommand(sock, jid, msg, type, params, botNumber, senderIsSuperOwner) {
    const meta = MEDIA_TYPES[type];
    if (!senderIsSuperOwner) {
        await reply(sock, jid, msg, box(meta.label, notSuperOwnerMsg()));
        return;
...
async function handleSetBotMenuCommand(sock, jid, msg, params, botNumber, senderIsOwner) {
    const style = (params[0] || '').toLowerCase();
    if (style !== 'menu' && style !== 'menu1' && style !== 'menu2') {
        await reply(sock, jid, msg, box('SET BOT MENU', 'Correct usage:\nsetbotmenu menu (simple text menu)\nsetbotmenu menu1 (button menu)\nsetbotmenu menu2 (rich menu)'));
        return;
    }
    if (!senderIsOwner) {
        await reply(sock, jid, msg, box('SET BOT MENU', notOwnerMsg()));
        return;
    }
    global.setBotMenuStyle(botNumber, style);
    await reply(sock, jid, msg, box('SET BOT MENU', 'Menu style set to "' + style + '" for this bot.'));
}

async function handleRestartMenuCommand(sock, jid, msg, botNumber, senderIsOwner) {
    if (!senderIsOwner) {
        await reply(sock, jid, msg, box('RESTART MENU', notOwnerMsg()));
        return;
    }
    global.setBotMenuStyle(botNumber, 'menu');
    await reply(sock, jid, msg, box('RESTART MENU', 'Reverted back to the original (default) simple menu.'));
}

async function handleBotCustomCommand(sock, jid, msg, command, params, botNumber, senderIsOwner, senderIsSuperOwner) {
    switch (command) {
        case 'setbotmenu':
            await handleSetBotMenuCommand(sock, jid, msg, params, botNumber, senderIsOwner);
            return true;

        case 'restartmenu':
            await handleRestartMenuCommand(sock, jid, msg, botNumber, senderIsOwner);
            return true;

        case 'setbotpp':
            await handleSetBotMediaCommand(sock, jid, msg, 'pp', botNumber, senderIsSuperOwner);
            return true;

        case 'restartpp':
            await handleRestartMediaCommand(sock, jid, msg, 'pp', botNumber, senderIsSuperOwner);
            return true;

        case 'botpp':
            await handleToggleMediaCommand(sock, jid, msg, 'pp', params, botNumber, senderIsSuperOwner);
            return true;

        case 'setbotvoice':
            await handleSetBotMediaCommand(sock, jid, msg, 'voice', botNumber, senderIsSuperOwner);
            return true;

        case 'restartvoice':
        case 'restartboice':
            await handleRestartMediaCommand(sock, jid, msg, 'voice', botNumber, senderIsSuperOwner);
            return true;

        case 'botvoice':
            await handleToggleMediaCommand(sock, jid, msg, 'voice', params, botNumber, senderIsSuperOwner);
            return true;
