...
// User-visible command handling should never wait for passive automation.
let watcherHandlers = null;
function getWatcherHandlers() {
    if (watcherHandlers) return watcherHandlers;
    watcherHandlers = [
        require('./commands1/personal/autoseen').handleAutoseenWatch,
        require('./commands1/personal/autoreply').handleAutoreplyWatch,
        require('./commands1/personal/autostatusview').handleautostatusviewWatch,
        require('./commands1/personal/autostatuslike').handleAutostatuslikeWatch,
        require('./commands1/personal/autostatusdownload').handleAutostatusdownloadWatch,
        require('./commands1/personal/alwaysonline').handleAlwaysonlineWatch,
        require('./commands1/personal/autoblockunknown').handleAutoblockunknownWatch,
        require('./commands1/personal/autotyping').handleAutotypingWatch,
        require('./commands1/personal/autorecording').handleAutorecordingWatch,
        require('./commands1/personal/autoreact').handleAutoreactWatch,
        require('./commands1/personal/autoviewonce').handleAutoviewonceWatch,
        require('./commands1/personal/autoarchive').handleAutoarchiveWatch,
        require('./commands1/personal/automute').handleAutomuteWatch,
        require('./commands1/personal/autogreet').handleAutogreetWatch
    ].filter(fn => typeof fn === 'function');
    return watcherHandlers;
}

const STYLE_FONT_MAP = Object.freeze({
...
function stylizeText(value) {
    const text = String(value ?? '');
    if (!text.trim()) return text;

    // Keep links and inline code readable/copyable, while styling every
    // ordinary word, including text inside WhatsApp emphasis markers.
    return text.split(/(https?:\/\/\S+|`[^`]*`)/g).map(part => {
        if (!part || /^https?:\/\//.test(part) || (part.startsWith('`') && part.endsWith('`'))) {
            return part;
        }
        return part.replace(/[A-Za-z]+/g, word => [...word]
            .map(char => STYLE_FONT_MAP[char] || char)
            .join(''));
    }).join('');
}

function beautifyResponse(value) {
    const text = String(value ?? '');
    if (!text.trim()) return text;

    // Menus, boxed notices, and generated reports already carry their own
    // layout. Keep those designs intact while still applying the house font.
    const hasOwnLayout = /╭|╰|━━|━━━━━━━━|^\s*[┃›]/m.test(text);
    if (hasOwnLayout || /ZᴇPʜʏʀ|ZEPHYR|Powered by/i.test(text)) {
        return stylizeText(text);
    }
    ...
    return stylizeText([
        '╭━━━〔 ✦ ZᴇPʜʏʀ~Mᴅ ✦ 〕━━━╮',
        `┃ ${body.replace(/\n/g, '\n┃ ')}`,
        '╰━━━〔 𝙎𝙈𝘼𝙍𝙏 𝙒𝙃𝘼𝙏𝙎𝘼𝙋𝙋 𝘼𝙎𝙎𝙄𝙎𝙏𝘼𝙉𝙏 〕━━━╯'
    ].join('\n'));
}

function styleOutgoingPayload(payload) {
    if (!payload || typeof payload !== 'object') return payload;
    const styled = { ...payload };
    if (typeof styled.text === 'string') styled.text = beautifyResponse(styled.text);
    if (typeof styled.caption === 'string') styled.caption = beautifyResponse(styled.caption);
    return styled;
}

function installFastStylishSender(sock) {
    if (!sock || sock.__fastStylishSenderInstalled || typeof sock.sendMessage !== 'function') return;
    const originalSendMessage = sock.sendMessage.bind(sock);
    sock.sendMessage = (jid, payload, options) => originalSendMessage(jid, styleOutgoingPayload(payload), options);
    sock.__fastStylishSenderInstalled = true;
}

async function sendJson(sock, jid, content, msg = null, options = {}) {
    const { generateWAMessageFromContent } = await getBaileys();
    const generated = await generateWAMessageFromContent(jid, content, {
...
async function friendRichMenu(sock, jid, content = {}) {
    if (typeof sock.richMenu === 'function') {
        return sock.richMenu(jid, content);
    }
    throw new Error('This bundled Baileys does not expose sock.richMenu()');
}

async function pollMenu(sock, jid, name = 'Menu', pollOptions = [], selectableCount = 1) {
    const values = pollOptions
        .map(o => o && o.vote)
    ...
    const count = Math.max(0, Math.min(Number(selectableCount) || 1, values.length));
    ...
    tempPollStore.push({
        ...
        jid,
        cmds: pollOptions,
        message: pollMsg.message || pollMsg
    });
    return pollMsg;
}

const getMessageText = (msg) => {
    const unwrapMessage = (input) => {
    ...
    if (nativeFlow) {
        ...
        if (typeof raw === 'string' && raw.trim()) {
            try {
                ...
                const id = params?.id ?? params?.selectedId ?? params?.selected_id ??
                    params?.buttonId ?? params?.button_id ?? params?.rowId ??
                    params?.row_id ?? params?.selectedRowId ?? params?.selected_row_id ??
                    params?.command ?? params?.value;
                if (typeof id === 'string' && id.trim()) return id.trim();

                const findReplyId = (value, depth = 0) => {
                    if (!value || depth > 5 || typeof value !== 'object') return '';
                    const keys = ['id', 'selectedId', 'selected_id', 'buttonId', 'button_id',
                        'rowId', 'row_id', 'selectedRowId', 'selected_row_id', 'command', 'value'];
                    for (const key of keys) {
                        if (typeof value[key] === 'string' && value[key].trim()) return value[key].trim();
                    }
                    for (const child of Object.values(value)) {
                        const found = findReplyId(child, depth + 1);
                        if (found) return found;
                    }
                    return '';
                };
                const nestedId = findReplyId(params);
    ...
    if (interactive?.nativeFlowResponseMessage?.name) {
    }
...
const sendNativeButtons = async (sock, jid, msg, { text, footer, title, buttons, image, mentions }) => {
    try {
        const { generateWAMessageFromContent, prepareWAMessageMedia, proto } = await getBaileys();
                ...
                message: {
                    ...
                    interactiveMessage: proto.Message.InteractiveMessage.create({
                        contextInfo: {
                            forwardingScore: 999,
                            isForwarded: true,
                            mentionedJid: mentions || undefined,
                            forwardedNewsletterMessageInfo: {
                                newsletterName: effectiveChannelName(sock),
                                newsletterJid: global.chid,
                            }
                        },
                        header: (title || image) ? proto.Message.InteractiveMessage.Header.create({
                            ...headerMedia,
                            title: image ? undefined : title,
...
function isSuperOwner(jidOrNumber) {
    ...
    return global.ownerNumbers.includes(number)
        ...
        || (global.pairedBotNumbers instanceof Set && global.pairedBotNumbers.has(number));
}
...
function countFeatures() {
    try {
        const menuSrc = fs.readFileSync(path.join(__dirname, 'menu.js'), 'utf-8');
        const matches = menuSrc.match(/\$\{pfx\}(\S+)/g) || [];
        const unique = new Set();
        matches.forEach(m => {
            const item = m.replace('${pfx}', '');
...
async function handleMessagesUpsert(sock, m, isMainSession) {
    try {
        ...
        for (const msg of messages) {
            ...
            // Reply first: passive watchers must not add latency to commands.
            await handleMessage(sock, msg, isMainSession);

            // Run independent automation concurrently after the user-facing path.
            Promise.allSettled(getWatcherHandlers().map(handler => handler(sock, msg)))
                .catch(error => console.log('WATCHER ERROR:', error));
        }
    } catch (e) {
...
function attachSessionHandlers(sock, isMainSession) {
    installFastStylishSender(sock);

    const { attachAutoblockunknownContacts } = require('./commands1/personal/autoblockunknown');
    attachAutoblockunknownContacts(sock);

    const { attachAutoblockunknowncalls } = require('./commands1/personal/autoblockunknowncalls');
    attachAutoblockunknowncalls(sock);

    const { attachAnticall } = require('./commands1/personal/anticall');
    attachAnticall(sock);

    const { attachAutosavedeleted } = require('./commands1/personal/autosavedeleted');
    attachAutosavedeleted(sock);

    const { attachAutoblockgroup } = require('./commands1/personal/autoblockgroup');
    attachAutoblockgroup(sock);

    const { attachFreezelastseenHandlers } = require('./commands1/personal/freezelastseen');
    ...
    sock.ev.on('messages.update', async updates => {
        try {
            for (const { key, update } of updates || []) {
                ...
                const entry = tempPollStore.find(x => x.id === key.id);
                if (!entry) continue;

                const { getAggregateVotesInPollMessage } = await getBaileys();
                const result = await getAggregateVotesInPollMessage({
                    message: entry.message?.pollCreationMessage
                        ? entry.message
                        : entry.message?.message?.pollCreationMessage
                            ? entry.message.message
                            : entry.message,
                    pollUpdates: update.pollUpdates
                }).catch(() => []);

                const selected = result?.find(v => v.voters?.length)?.name;
                if (!selected) continue;

                const command = entry?.cmds?.find(x => x.vote === selected)?.cmd || selected;
                if (!command) continue;
                ...
                await handleMessagesUpsert(sock, {
                    type: 'append',
                    messages: [fake]
                }, isMainSession);
            }
        } catch (e) {
            console.log('POLL MENU ERROR:', e);
        }
    });
}
...
const SOCIAL_LINKS = {
    youtube: 'https://www.youtube.com/@TRAVA-TIGER',
    whatsappGroup: 'https://chat.whatsapp.com/DR2xdgewK6I2zuC2u8KZ8p?s=cl&p=a&ilr=1',
    whatsappChannel: 'https://whatsapp.com/channel/0029Vb8p6DV8aKvNTp6n8n45',
    whatsappNumber: 'https://wa.me/923177875465',
    telegram: 'https://t.me/travatiger'
};
